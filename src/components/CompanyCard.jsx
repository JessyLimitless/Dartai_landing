...
import FinancialSeriesModal from './FinancialSeriesModal'
...
export default function CompanyCard({ corpCode, onBack, onViewCard }) {
  ...
  return (
    <div className="page-container page-enter" style={{ maxWidth: 640, margin: '0 auto', padding: '0 0 80px', backgroundColor: colors.bgPrimary }}>
      ...
      {/* 3-1. 재무 시각화 진입 */}
      <FinancialSeriesButton
        corpCode={corpCode}
        corpName={header.corp_name || ''}
        colors={colors}
        dark={dark}
      />
...
function FinancialSeriesButton({ corpCode, corpName, colors, dark }) {
      ...
      <div style={{ padding: '0 20px 20px' }}>
        <button
          ...
          style={{
            ...
            padding: '13px 16px',
            backgroundColor: dark ? '#1C1C1F' : '#F7F7F8',
        ...
        >
          <svg width="17" height="17" viewBox="0 0 24 24" fill="none"
            stroke={PREMIUM.accent} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M3 3v18h18" />
          ...
          재무 시각화
          <span style={{ fontSize: 12, fontWeight: 400, color: colors.textMuted }}>
            12분기 실적·재무·현금흐름
          </span>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor"
            strokeWidth="2" strokeLinecap="round"
            style={{ marginLeft: 'auto', color: colors.textMuted, flexShrink: 0 }}>
            <path d="m9 18 6-6-6-6" />
          </svg>
        </button>
      </div>
      {open && (
        <FinancialSeriesModal
          corpCode={corpCode}
          corpName={corpName}
