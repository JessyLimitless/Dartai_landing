...
const ISSUES = [
  {
    ...
    analysis: `## 제주반도체 — 거인이 무시한 시장을 독점하는 자

### 기업 개요

제주반도체(080220) | 코스닥 | 시총 1.27조 | 저전력 메모리 팹리스

2000년 설립, 2005년 코스닥 상장. 대표 박성식 (삼성전자 반도체 출신). **LPDDR(저전력 DRAM) 매출 비중 70%.** 공장 없이 설계만 하고 SK하이닉스 등에 생산을 맡기는 팹리스 모델.

핵심 경쟁력은 단 하나다: **삼성·SK하이닉스·마이크론이 진입하지 않는 시장을 독점한다.**

대형 3사는 고용량·고성능 DRAM(DDR5, HBM)에 집중한다. 와이파이 공유기, 카드 결제 단말기, 통신 중계기, IoT 기기에 들어가는 **저용량·저전력 메모리**는 그들에게 "돈이 안 되는" 시장이다. 제주반도체는 그 틈새에서 세계 3위다.

---

### 왜 지금 주목해야 하는가 — 온디바이스 AI

CES 2026의 키워드는 "피지컬 AI"였다. 로봇이 스스로 판단하고, 스마트폰이 클라우드 없이 AI를 돌리고, 자동차가 엣지에서 추론한다. **이 모든 것의 공통 조건: 저전력.**

클라우드 AI는 전력을 무제한으로 쓸 수 있다. 온디바이스 AI는 배터리로 돌아야 한다. LPDDR(Low Power DDR)이 필수인 이유다.

| 시장 | LPDDR 필요성 | 성장률 |
|------|-------------|--------|
| 5G IoT (스마트미터, 산업용) | 필수 | 25%+ CAGR |
| 엣지 AI (카메라, 로봇) | 필수 | 30%+ CAGR |
| 커넥티드 차량 | 필수 | 20%+ CAGR |
| 와이파이 7 공유기 | 필수 | 15%+ CAGR |

제주반도체는 이 시장의 **모든 전방위 고객**에게 납품한다. 매출 상위 10개사가 60~70%로, 특정 고객 쏠림이 제한적이다.

---

### 퀄컴 + 미디어텍 듀얼 인증 — 국내 유일

LPDDR을 만들어도 AP(Application Processor)와 호환 인증을 받지 못하면 의미가 없다. 전세계 AP 시장은 퀄컴과 미디어텍이 양분한다.

**제주반도체는 퀄컴과 미디어텍 양사 인증을 모두 획득한 국내 유일 기업이다.** 글로벌로도 마이크론과 함께 둘뿐이다.

이 인증이 해자(moat)다. 인증 획득에 2~3년이 걸리고, 대형 3사가 이 시장에 진입할 인센티브가 없으니, 기존 인증 보유 기업의 독점이 강화된다.
...
| 부채비율 | 25.3% | 22.4% | **45.1%** | 투자 확대 |

2024년에 OPM이 5.9%로 떨어졌다가 2025년 11.9%로 회복. 매출이 86% 폭증하면서 이익이 따라왔다. 분기별로 보면 더 극적이다:
...
Q3에서 분기 매출 1,000억을 처음 돌파했다. 전년 동기 대비 +197%. 이건 일회성이 아니다 — DDR4 단종에 따른 공급 부족이 구조적으로 제주반도체에 유리한 환경을 만들고 있다.

---

### CB+BW 1,170억 — 희석인가, 투자인가

2026년 3월 13일, CB 450억 + BW 720억 = **총 1,170억원** 발행 결정.

| 구분 | 규모 | 용도 |
|------|------|------|
| CB 9회 | 450억 | 웨이퍼 300억 + R&D 150억 |
| BW #1 | 620억 | 원자재 + R&D |
| BW #2 | 100억 | 해외 거래처(SUNNY RISE) |

전환가/행사가 **44,300원** — 현재가(36,850원) 대비 **20% 프리미엄.**

희석 규모: CB 101.6만주 + BW 162.5만주 = **264만주 (기존 발행주식의 7.7%)**

**해석:** 전환가가 현재가보다 20% 높다는 것은 발행 당시 "이 가격 이상으로 갈 것"이라는 경영진의 확신이다. 그리고 용도가 "웨이퍼 구입"이라는 것은 수요가 생산 능력을 초과하고 있다는 뜻이다. **팔 물건이 있는데 재료가 부족해서 돈을 빌리는 것** — 이건 긍정적 희석이다.
...
PER 41배는 성장주로서 합리적이지만, 65배(EPS 567원 기준)면 부담스럽다. 핵심은 **성장률이 PER을 정당화하는가**다.

2025년 매출 +86%, 영업이익 +274%. 이 성장률이 2~3년 유지되면 PER은 급속히 낮아진다:
...
Bull 시나리오에서 Forward PER 23배 — 반도체 팹리스 업종 평균(25~30배)보다 낮다.
...
| 기업 | 시총 | 포지션 |
|------|------|--------|
| ESMT (대만) | - | 저전력 메모리 세계 1위 |
| ISSI (미국) | - | 세계 2위 |
| **제주반도체** | **1.27조** | **세계 3위, 1위 목표** |

대형 3사(삼성/SK/마이크론)가 DDR4를 단종하면서, 저전력 DDR4 시장에 공급 공백이 생겼다. 제주반도체는 이 공백을 채우며 점유율을 확대 중이다.
...
|--------|------|------|------|
| CB/BW 희석 7.7% | 확정 | 중 | A |
| 대형사 니치마켓 재진입 | 낮음 | 극대 | B |
| 대주주 지분 10.47% — 경영권 불안 | 중 | 대 | A |
| 외국인 0.22% — 유동성 리스크 | 높음 | 중 | A |
| LPDDR4X/5 양산 지연 | 중 | 대 | A |
| 원자재(웨이퍼) 가격 상승 | 중 | 중 | B |
...
1. **LPDDR4X 양산 일정** — SK하이닉스 팹 투입 시점
2. **분기 실적 지속성** — Q3 1,110억이 일회성인지 구조적인지
3. **CB/BW 전환 시점** — 전환가 44,300원 도달 여부
4. **외국인 매수 유입** — 0.22%에서 변화 시 신호
5. **퀄컴 Snapdragon 차기 칩에 LPDDR5 탑재** — 제주반도체 인증 포함 여부
